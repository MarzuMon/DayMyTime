...
export function scheduleNotification(schedule: Schedule, onUpdate?: () => void) {
  ...
  const reminderMin = getPreReminderMinutes();
  if (reminderMin > 0) {
...
export function cancelNotification(id: string) {
  ...
  if (preTimer) { clearTimeout(preTimer); preReminderTimers.delete(id); }
}
...
function showPreReminder(schedule: Schedule, minutes: number) {
  ...
  const catEmoji = schedule.category === "meeting" ? "🤝" : schedule.category === "class" ? "📚" : schedule.category === "work" ? "💼" : "📌";
  const body = schedule.meetingLink
    ? `Starting in ${minutes} minutes\n🔗 ${schedule.description || "Get ready!"}`
    : `Starting in ${minutes} minutes\n${schedule.description || "Get ready!"}`;
  const notification = new Notification(`⏰ ${catEmoji} ${schedule.title} — Soon!`, { body, icon: "/favicon.ico", tag: `${schedule.id}-pre` });
  notification.onclick = () => {
    window.focus();
    if (schedule.meetingLink) window.open(schedule.meetingLink, "_blank");
    notification.close();
  };
}
...
function snoozeSchedule(schedule: Schedule, onUpdate?: () => void) {
  ...
  scheduleNotification({ ...schedule, scheduledTime: new Date(Date.now() + snoozeMin * 60 * 1000).toISOString() }, onUpdate);
}
...
function showNotification(schedule: Schedule, onUpdate?: () => void) {
  ...
  if (prefs.soundEnabled) {
    ...
    } catch (e) { console.warn("Could not play alarm tone:", e); }
  }
  ...
  const notification = new Notification(`${catEmoji} ${schedule.title}`, { body, icon: "/favicon.png", tag: schedule.id, requireInteraction: true });
  ...
  notification.onclose = () => {
    stopAlarmTone();
    if (!userClicked && prefs.autoSnooze) snoozeSchedule(schedule, onUpdate);
  };

  if (schedule.duration && prefs.durationAlert) {
    window.setTimeout(() => {
      new Notification(`✅ ${schedule.title} — Duration Complete`, {
        body: `${schedule.duration} minutes have passed`, icon: "/favicon.ico", tag: `${schedule.id}-done`,
      });
